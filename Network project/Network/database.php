				  ...
				  <div class="tile tile-hot" >
					...
					<h3 class="tile-title">
							...
							if($row[0] == NULL) {
								...
								else if ($row['usage'] > 2147483648){
									$check = 2;
									echo "<img src='images/icons/svg/loop.svg' alt='Chat' class='tile-image' style='height:80px; width:80px;'> <br> <span class='fui-cross'></span> Your file size exceded the limit !!!";
								}
								else 									
									$check = 1;
							}
							...
							if($flag==1 AND ($check==0 OR $check ==2)){
								if( $check == 2)
									...
									$sql = "UPDATE  `network`.`user` SET  `usage` =  '".$usage."' WHERE  `user`.`email` =  '".$var4."';";
									if(!mysqli_query($con, $sql)){
										echo "<br> Problem in updating usage to ".$usage;
										}
									else {
